...
namespace StudentMDataAccess
{
    public class GradeDataAccess
    {
        public DataTable GetListAdo()
        {
            ...
            //string sql = "select stuInfo.id as 'id',stuInfo.name as 'name',sum(case courseInfo.kch when '1' then gradeInfo.ach else 1 end) as 'ach1',sum(case courseInfo.kch when '2' then gradeInfo.ach else 0 end) as 'ach2', sum(case courseInfo.kch when '3' then gradeInfo.ach else 0 end) as 'ach3', sum(case courseInfo.kch when '4' then gradeInfo.ach else 0 end) as 'ach4',sum(gradeInfo.ach) as 'ach5' from gradeInfo, stuInfo, courseInfo  where(gradeInfo.id = stuInfo.id) and(gradeInfo.kch = courseInfo.kch)  group by stuInfo.id,stuInfo.name";
            string sql = "select stuInfo.id as 'id',stuInfo.name as 'name',sum(case courseInfo.kch when '0001' then gradeInfo.ach else 0 end) as 'ach1',sum(case courseInfo.kch when '0002' then gradeInfo.ach else 0 end) as 'ach2', sum(case courseInfo.kch when '0003' then gradeInfo.ach else 0 end) as 'ach3', sum(case courseInfo.kch when '0004' then gradeInfo.ach else 0 end) as 'ach4',sum(gradeInfo.ach) as 'ach5' from gradeInfo, stuInfo, courseInfo  where(gradeInfo.id = stuInfo.id) and(gradeInfo.kch = courseInfo.kch)  group by stuInfo.id,stuInfo.name";
            //string sqltest = ""
        ...
        public DataTable SelectData(Grade grade)
        {
            ...
            if (grade.kch == null)
            {
                sql = "select stuInfo.id as 'id',stuInfo.name as 'name'," +
                    "sum(case courseInfo.kch when '0001' then gradeInfo.ach else 0 end) as 'ach1'," +
                    "sum(case courseInfo.kch when '0002' then gradeInfo.ach else 0 end) as 'ach2', " +
                    "sum(case courseInfo.kch when '0003' then gradeInfo.ach else 0 end) as 'ach3', " +
                    "sum(case courseInfo.kch when '0004' then gradeInfo.ach else 0 end) as 'ach4'," +
                    "sum(gradeInfo.ach) as 'ach5'" +
                    "from gradeInfo, stuInfo, courseInfo" +
                    "where(gradeInfo.id = stuInfo.id) and((gradeInfo.id = '" + grade.id + "')or('" + grade.id + "' = null)) and(gradeInfo.kch = courseInfo.kch) and" +
                    "(gradeInfo.id in" +
            ...
            else
            {
                ...
                sql = "select stuInfo.id as 'id',stuInfo.name as 'name'," +
                    ...
                    "(gradeInfo.id in" +
                    "(select id from gradeInfo" +
                    "where ((ach >= " + grade.achlow + ")and(ach <= " + grade.achhigh + ") and (gradeInfo.kch = '" + grade.kch + "'))))" +
                    "group by stuInfo.id,stuInfo.name";
            }          
            DataTable dt = DBUtil.GetDataTable(sql);
            return dt;
